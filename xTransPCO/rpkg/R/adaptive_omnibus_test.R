# Core TANGO test for one SNP-module pair.
...
validate_inputs <- function(z, corr = NULL) {
  ...
  if (is.null(corr)) {
    ...
    corr <- as.matrix(corr)
    if (!all(dim(corr) == c(length(z), length(z)))) {
      stop("corr must have dimension length(z) by length(z)")
    }
  }
...
#'
#' @param z Numeric vector of SNP-feature Z scores for one module.
#' @param corr Optional feature correlation matrix.
#' @param network Optional biological network matrix.
#' @param component_weights Optional named weights for ACAT combination.
#' @return A list containing component p-values and the final p-value.
#' @export
tango_test <- function(z, corr = NULL, network = NULL, component_weights = NULL) {
  ...
  final_p <- acat(pvals, weights = weights)
