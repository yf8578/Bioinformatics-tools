...
This script generates SNP-module Z-score vectors under known effect
...
The output is a tidy TSV table that can be summarized in R or Python.
"""
...
def chain_network(k: int) -> np.ndarray:
    """Create a simple chain graph for network-localized simulations."""
...
def simulate_one(k, mode, effect, prop, rho, rng):
    """Simulate one Z vector and truth labels."""
...
def run_methods(z, corr, network=None):
    """Run all benchmark methods on the same input."""
