...
These functions are deliberately separated from the TANGO test so that benchmark
scripts can compare several methods on exactly the same input Z vector and
feature correlation matrix.

The `pco_acat_test` function is a lightweight PCO-like baseline. It is not a
claim of exact reimplementation of the original trans-PCO paper. Exact
comparison should use the original trans-PCO code if available.
"""
...
def pc1_test(z, corr=None):
    ...
    Project the Z vector onto the first eigenvector of the feature correlation
    matrix and test the projected statistic as approximately standard normal.
    """
...
def pco_acat_test(z, corr=None, eigen_threshold=0.1):
    ...
    2. project Z onto PCs with eigenvalue above a threshold;
    3. compute two-sided p-value for each standardized PC score;
    4. combine PC-level p-values by ACAT.

    This is a PCO-like baseline for development benchmarks, not an exact copy of
    the six-test PCO implementation used in the original trans-PCO paper.
    """
